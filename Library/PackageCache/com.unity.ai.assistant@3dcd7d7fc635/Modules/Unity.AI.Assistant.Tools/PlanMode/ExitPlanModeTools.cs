...
namespace Unity.AI.Assistant.Tools.Editor
{
    class ExitPlanModeTools
    {
        ...
        internal static async Task<string> ExitPlanMode(
            ...
            string title = null)
        {
            var resolvedPath = PlanModeTools.ResolvePath(planPath);

            PlanModeTools.ValidatePlansDirSecurity(resolvedPath);

            if (!File.Exists(resolvedPath))
                throw new InvalidOperationException($"Plan file not found at '{planPath}'. Use Unity.WritePlan to create new plan files before calling Unity.ExitPlanMode.");
